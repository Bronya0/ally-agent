...
const (
	defaultScheduledTaskSteps   = 100
	maxScheduledTaskSteps       = 1000
	defaultScheduledTaskTimeout = 3600
...
func parseScheduledTaskSchedule(value, timezone string) (ScheduledTaskSchedule, error) {
	value, timezone = strings.TrimSpace(value), strings.TrimSpace(timezone)
	if value == "" {
		return ScheduledTaskSchedule{}, codedToolError("E_SCHEDULED_TASK_SCHEDULE", errors.New("schedule is required"))
	}
	if _, err := time.Parse(time.RFC3339, value); err == nil {
		return ScheduledTaskSchedule{Type: "once", At: value, Timezone: timezone}, nil
	}
	if duration, err := time.ParseDuration(value); err == nil {
		if duration < minScheduledTaskInterval {
			return ScheduledTaskSchedule{}, codedToolError("E_SCHEDULED_TASK_INTERVAL", fmt.Errorf("interval must be at least %s", minScheduledTaskInterval))
		}
		return ScheduledTaskSchedule{Type: "interval", Every: value, Timezone: timezone}, nil
	}
	return ScheduledTaskSchedule{Type: "cron", Cron: value, Timezone: timezone}, nil
}
...
func (m *scheduledTaskManager) registerLocked(task *ScheduledTask, now time.Time) error {
	...
	case "interval":
		...
		return nil
	case "cron":
		spec := strings.TrimSpace(task.Schedule.Cron)
		if task.Schedule.Timezone != "" {
			spec = "CRON_TZ=" + task.Schedule.Timezone + " " + spec
		}
...
func normalizeScheduledTask(task *ScheduledTask, now time.Time) error {
	...
	task.Schedule.Type = strings.ToLower(strings.TrimSpace(task.Schedule.Type))
	task.Schedule.At = strings.TrimSpace(task.Schedule.At)
	task.Schedule.Every = strings.TrimSpace(task.Schedule.Every)
	task.Schedule.Cron = strings.TrimSpace(task.Schedule.Cron)
	task.Schedule.Timezone = strings.TrimSpace(task.Schedule.Timezone)
	if task.ID == "" {
	...
	if task.MaxSteps <= 0 {
		task.MaxSteps = defaultScheduledTaskSteps
	}
	if task.MaxSteps > maxScheduledTaskSteps {
		return codedToolError("E_SCHEDULED_TASK_STEPS", fmt.Errorf("maxSteps must be <= %d", maxScheduledTaskSteps))
	}
	if task.TimeoutSeconds <= 0 {
		task.TimeoutSeconds = defaultScheduledTaskTimeout
	}
	if task.TimeoutSeconds < 60 || task.TimeoutSeconds > maxScheduledTaskTimeout {
		return codedToolError("E_SCHEDULED_TASK_TIMEOUT", fmt.Errorf("timeoutSeconds must be between 60 and %d", maxScheduledTaskTimeout))
	}
	switch task.Schedule.Type {
	case "once":
		if task.Schedule.At == "" {
			return codedToolError("E_SCHEDULED_TASK_AT", errors.New("schedule.at is required for once"))
		}
		if _, err := time.Parse(time.RFC3339, task.Schedule.At); err != nil {
			return codedToolError("E_SCHEDULED_TASK_AT", fmt.Errorf("invalid RFC3339 time: %w", err))
		}
	case "interval":
		duration, err := time.ParseDuration(task.Schedule.Every)
		...
		if duration < minScheduledTaskInterval {
			return codedToolError("E_SCHEDULED_TASK_INTERVAL", fmt.Errorf("interval must be at least %s", minScheduledTaskInterval))
		}
	case "cron":
		if task.Schedule.Cron == "" {
			return codedToolError("E_SCHEDULED_TASK_CRON", errors.New("schedule.cron is required for cron"))
		}
		if task.Schedule.Timezone != "" {
			if _, err := time.LoadLocation(task.Schedule.Timezone); err != nil {
				return codedToolError("E_SCHEDULED_TASK_TIMEZONE", fmt.Errorf("invalid timezone: %w", err))
			}
		}
