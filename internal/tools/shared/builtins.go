...
func chatToolsUncached() []openai.Tool {
	return []openai.Tool{
		...
		functionTool("grep", "Search UTF-8 file contents with ripgrep (`rg`); returns exact match counts plus sample match lines grouped by file, sorted by relevance. Case-insensitive by default; use `caseSensitive: true` or prefix the pattern with `(?-i)` for exact-case. `glob`: no slash matches the basename (`*.go`), with slash matches a relative path (`frontend/src/*.vue`). `contextBefore`/`contextAfter` (0-50) add surrounding context lines, often avoiding a separate `read`. On sample truncation, page with the returned `nextOffset` as `offset`; count/occurrence/file stats stay exact. Re-read a file before using a sampled line as edit source text, since long lines may be truncated. Workspace-relative paths resolve under the workspace; explicit absolute paths allowed for read-only search.", map[string]any{
			"type": "object",
			"properties": map[string]any{
