...
func chatToolsUncached() []openai.Tool {
	return []openai.Tool{
		...
		functionTool("grep_files", "Search UTF-8 text file contents using ripgrep (`rg`); returns exact stats plus sample match lines. Result fields: `matchedLines` = number of matching LINES, `hits` = total number of regex HITS (use this for \"how many times does X appear\" questions), `files` = matching file count; these stats are always exact. `fileHits` groups sample matches by file path (each `path` + `matches[{lineNum, content}]`); every `content` is capped at 200 bytes plus an ellipsis when shortened, and `contentTruncated: true` marks a truncated line, so re-read the file before using a line as edit source text. `truncated`/`samplesTruncated` mean sample collection hit a cap; the counts above remain exact. Search is case-insensitive by default; prefix the pattern with `(?-i)` to make it case-sensitive. `glob` filters files: a pattern without a slash matches the basename at any depth (`*.go`), a pattern with a slash matches a relative path (`frontend/src/*.vue`). Workspace-relative paths are resolved under the workspace; explicit absolute paths are allowed for read-only search subject to safety checks. Skips binary/large/heavy directories. Error results include errorCode values such as E_GREP_REGEX, E_GREP_GLOB, E_GREP_TIMEOUT, E_GREP_PATH, E_SEARCH_ROOT_BLOCKED, and E_RIPGREP_NOT_FOUND.", map[string]any{
			"type": "object",
			"properties": map[string]any{
