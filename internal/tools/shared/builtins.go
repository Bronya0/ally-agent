...
func chatToolsUncached() []openai.Tool {
	return []openai.Tool{
		...
		functionTool("edit", "Validate and apply exact replacements across multiple workspace files in one call. Each change is matched against the original snapshot of its file; all sources are located before any write, then applied backwards, so earlier edits never shift later line numbers. Source regions cannot overlap, and all files are validated before any write. Read returns numbered text with a `version`; every file requires the current `version` from read, and a stale one fails with E_VERSION_MISMATCH. Prefer a small exact unique `oldText` for each change; `replace_all` replaces every non-overlapping exact occurrence; `lineRange` is for larger whole-line blocks. See each change field for the full rules. If exact matching fails, Ally retries once ignoring invisible differences: trailing spaces, smart quotes become ASCII quotes, Unicode dashes become ASCII hyphens, and special spaces are normalized; the touched lines are then rewritten. A successful normalized match returns a warning; if it is ambiguous the call fails with E_MULTI_MATCH — add surrounding context instead of re-reading the whole file. Error codes: E_BAD_EDIT (malformed request, overlap, non-unique oldText, too many changes), E_VERSION_MISMATCH (file changed since read; re-read all affected files and retry with the new version), E_PATH_OUTSIDE.", map[string]any{
			"type": "object",
			"properties": map[string]any{
		...
		functionTool("grep", "Search UTF-8 text file contents using ripgrep (`rg`); returns exact stats plus sample match lines. Result fields: `matchedLines` = number of matching LINES, `hits` = total number of regex HITS (use this for \"how many times does X appear\" questions), `files` = matching file count; these stats are always exact. `fileHits` groups sample matches by file path (each `path` + `matches[{lineNum, content, context?}]` + exact `matchCount` per file), **sorted by `matchCount` descending so the most relevant file comes first — read the first entry first**; every `content` is capped at 200 bytes plus an ellipsis when shortened, and `contentTruncated: true` marks a truncated line, so re-read the file before using a line as edit source text. `fileCounts` lists every matched file with its exact hit count, sorted by count descending (top hotspots first, capped at 100 with `fileCountsTruncated` when larger). `truncated`/`samplesTruncated` mean sample collection hit a cap; the counts above remain exact. When samples are truncated, `nextOffset` is returned — pass it back as `offset` to page through the remaining matches. `offsetExhausted: true` means the requested `offset` skipped past the end of the match stream (`fileHits` is empty even though `hits` > 0) — reset `offset` to 0 to page from the beginning; a file group may carry `matchCount` with no sample lines when the sample quota was hit. `contextBefore`/`contextAfter` (0-50) include surrounding context lines, marked `context: true` and excluded from stats and offsets, which often avoids a separate `read`. Search is case-insensitive by default; set `caseSensitive: true` for exact-case matching, or prefix the pattern with `(?-i)` to make it case-sensitive. `glob` filters files: a pattern without a slash matches the basename at any depth (`*.go`), a pattern with a slash matches a relative path (`frontend/src/*.vue`). Workspace-relative paths are resolved under the workspace; explicit absolute paths are allowed for read-only search subject to safety checks. Skips binary/large/heavy directories. Error results include errorCode values such as E_GREP_REGEX, E_GREP_GLOB, E_GREP_TIMEOUT, E_GREP_PATH, E_SEARCH_ROOT_BLOCKED, and E_RIPGREP_NOT_FOUND.", map[string]any{
			"type": "object",
			"properties": map[string]any{
