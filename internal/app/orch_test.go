...
// TestServiceExitRetentionAndEviction 验证已结束服务的诊断保留：进程退出后
// 记录仍在 list/read 中可查（含 exitCode 与末尾输出），仅超过上限的旧记录被淘汰。
func TestServiceExitRetentionAndEviction(t *testing.T) {
	root := t.TempDir()
	app := NewApp()
	app.configPath = filepath.Join(root, "config.json")

	// Crash quickly with a distinctive final line, like a dev server failing
	// to bind a port.
	info, err := app.startServiceWithConfig(ConfigState{Workspace: root}, StartServiceRequest{
		Name:    "crash-test",
		Command: fmt.Sprintf("echo FATAL: simulated crash; exit 42"),
	})
	if err != nil {
		t.Fatal(err)
	}
	id := info.ID
	waitExited := func() *managedService {
		app.servicesMu.Lock()
		defer app.servicesMu.Unlock()
		if s := app.services[id]; s != nil {
			s.mu.Lock()
	...
	for waitExited() == nil && time.Now().Before(deadline) {
	...
	// The record must survive its process.
	listed := app.listServicesForTool()
	if len(listed.Services) != 1 {
		t.Fatalf("expected the exited service to remain listed, got %#v", listed)
	}
	summary := listed.Services[0]
	if summary.ID != id || summary.Status != "exited" || summary.ExitCode != 42 {
		t.Fatalf("expected exited record with exitCode 42, got %#v", summary)
	}

	// read on the dead service must return the crash line.
	res, err := app.readServiceOutput(ServiceReadRequest{ID: id})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(res.Output, "FATAL: simulated crash") {
		t.Fatalf("expected crash line in output, got %q", res.Output)
	}
	if res.Status != "exited" {
		t.Fatalf("expected status exited in read result, got %q", res.Status)
	}

	// Finished records evict oldest beyond maxFinishedServices. Start each
	// successor only after every running service exited so the transient
	// running count stays well under maxActiveServices.
	waitForIdle := func() bool {
		deadline := time.Now().Add(10 * time.Second)
		for time.Now().Before(deadline) {
			if app.listServicesForTool().ActiveCount == 0 {
				return true
			}
			time.Sleep(20 * time.Millisecond)
		}
	...
	for i := 0; i < maxFinishedServices; i++ {
		_, err := app.startServiceWithConfig(ConfigState{Workspace: root}, StartServiceRequest{
	...
	final := app.listServicesForTool()
	if len(final.Services) > maxFinishedServices {
		t.Fatalf("finished records must cap at %d, got %d", maxFinishedServices, len(final.Services))
	}
	for _, s := range final.Services {
		if s.ID == id {
			t.Fatalf("the oldest record (%s) should have been evicted, kept: %#v", id, s)
		}
	}
}

// TestStopServiceKeepsRecordReadable 验显 stop 后记录仍可读（模型在主动停止后
// 可能仍需检查末尾输出）。
func TestStopServiceKeepsRecordReadable(t *testing.T) {
	root := t.TempDir()
	app := NewApp()
	info, err := app.startServiceWithConfig(ConfigState{Workspace: root}, StartServiceRequest{
		Name:    "stop-test",
	...
	stopped, err := app.stopService(StopServiceRequest{ID: info.ID, GraceSeconds: 1})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := app.readServiceOutput(ServiceReadRequest{ID: info.ID}); err != nil {
		t.Fatalf("stopped service must stay readable: %v", err)
	}
}
