...
func sharedBatchStrategy() string {
	return "**Batch and parallelize where appropriate — balance round-trips and context hygiene**:\n" +
		...
		"- **Grep**: Use for fast path and line locating (lines mode groups matches by file — a bare path row, then indented `line: text` rows — with capped text previews of each matching line; only read the file when you need surrounding context). Paginate with `offset` using the `next-offset` attribute on the opening `<ally-grep>` tag; it resumes right after the last row shown. When searching for multiple keywords or patterns, emit `grep` calls concurrently in the same turn.\n" +
