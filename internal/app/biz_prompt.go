...
func sharedEditRules() string {
	return "1. Before a file's first edit, use `read` to obtain numbered content and its `version`. Text lines are displayed as `N: content`; the `N: ` prefix is not file content and must never be copied into edit text. During one continuous task, assume workspace files are not concurrently edited by another person; do not re-read a file merely for reassurance. After a successful edit, reuse its returned `version` only when the current source is known exactly. Re-read when the current source or line numbers are unknown, context compaction removed the reliable snapshot, or a formatter/generator/command or other external process may have changed the file.\n" +
		"2. In every `edit.files[].changes[]` item choose exactly one source. Prefer `lineRange` in inclusive `A-B` form (taken from the numbered `read` snapshot) for whole-line replacements; it is the default for multi-line blocks and needs no reproduced original text. Use a small, unique, unnumbered `oldText` only for an in-line edit inside one line, or when the target line is extremely long (e.g. minified JSON) and a line range is impractical. Never send both. `newText` contains only replacement source code/text, with no displayed line prefixes; empty `newText` deletes the selected source. With `lineRange`, `newText` replaces exactly the selected whole lines — lines outside the range (e.g. a closing brace on the next line) stay untouched, so never re-emit them; if the brace is inside the range, include it.\n" +
		"3. All `lineRange` values for one file refer to the original numbered `read` snapshot that produced its `version`. Ally locates all sources first and applies changes backwards, so do not adjust later line ranges after earlier insertions or deletions. Source regions must not overlap.\n" +
