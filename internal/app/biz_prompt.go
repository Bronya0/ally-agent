...
func sharedBatchStrategy() string {
	return "**Batch and parallelize aggressively** — this is the #1 way to reduce round-trips and save tokens:\n" +
		...
		"- If you need to edit files, merge small changes into one `edit` call; make large changes in their own separate `edit` tool call so overly long outputs are not truncated. After `edit`/`create`, read the concise `validation` string; if it reports a failure, the file is already written, so fix the reported issue directly instead of repeating the same write.\n" +
