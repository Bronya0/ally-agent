...
func sharedEditRules() string {
	return "1. Before a file's first edit, use `read` to obtain numbered content and its `version`. Text lines are displayed as `N: content`; the `N: ` prefix is not file content and must never be copied into edit text. During one continuous task, assume workspace files are not concurrently edited by another person; do not re-read a file merely for reassurance. After a successful edit, reuse its returned `version` only when the current source is known exactly. Re-read when the current source or line numbers are unknown, context compaction removed the reliable snapshot, or a formatter/generator/command or other external process may have changed the file.\n" +
		"2. In every `edit.files[].changes[]` item choose exactly one source. Prefer a small, unique, unnumbered `oldText` copied exactly from the numbered `read` snapshot for precise string replacement; this is the default for focused changes and also works for focused multi-line snippets when enough surrounding text makes the match unique. Use `lineRange` in inclusive `A-B` form only for larger whole-line replacements or when reproducing the exact source is impractical (for example, an extremely long single line). Never send both. `newText` contains only replacement source code/text, with no displayed line prefixes; empty `newText` deletes the selected source. With `oldText`, replace exact matched snippets; with `lineRange`, `newText` replaces exactly the selected whole lines — lines outside the range (e.g. a closing brace on the next line) stay untouched, so never re-emit them; if the brace is inside the range, include it.\n" +
		"3. By default, an effective `oldText` must occur exactly once in the original snapshot; include enough surrounding context to make it unique. The optional boolean `replace_all` defaults to `false`; when set to `true` with `oldText`, replace every non-overlapping exact occurrence from that same original snapshot. With `lineRange`, it is ignored and returned as a warning. `lineRange` values for one file refer to the original numbered `read` snapshot that produced its `version`. Ally locates all sources first and applies changes backwards, so do not adjust later ranges after earlier insertions or deletions. Source regions must not overlap.\n" +
		"4. Put all known changes across affected files in one `edit` call. Repeated normalized paths within one local call are merged when their versions match. Never send multiple file-mutation tool calls for the same path in one model response. Do not use patch, unified diff, or git apply.\n"
}
