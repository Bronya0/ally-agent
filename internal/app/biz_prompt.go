...
func sharedEditRules() string {
	return "1. Before a file's first edit, use `read` to obtain exact content and `version`. During one continuous task, assume workspace files are not concurrently edited by another person unless there is concrete evidence otherwise; do not re-read a file merely for reassurance. After a successful edit, reuse its returned `version` when the next exact `oldText` is already known. Re-read only when current content is unknown, context compaction removed the reliable snapshot, a formatter/generator/command or other external process may have changed the file, or a version/match error occurs.\n" +
		"2. Put all known changes across affected files in one `edit` call. Use exact, unique `oldText`; the schema defines the batch limits and replacement behavior. Each `oldText` must be non-empty, exact, unique in the original snapshot, and non-overlapping. Empty `newText` deletes `oldText`; insert by replacing a unique anchor with the anchor plus inserted content.\n" +
