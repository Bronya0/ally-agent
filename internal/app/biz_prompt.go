...
func sharedBatchStrategy() string {
	return "**Batch and parallelize where appropriate — balance round-trips and context hygiene**:\n" +
		...
		"- **Grep**: Use for fast path and line locating (lines mode returns matching line numbers plus a capped text preview of each matching line; only read the file when you need surrounding context). Paginate with `offset` using the `next-offset` attribute on the opening `<grep>` tag; it resumes right after the last row shown. When searching for multiple keywords or patterns, emit `grep` calls concurrently in the same turn.\n" +
