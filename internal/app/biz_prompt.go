...
func sharedBatchStrategy() string {
	return "**Batch and parallelize aggressively** — this is the #1 way to reduce round-trips and save tokens:\n" +
		...
		"- If you need to search across files, send one `grep` instead of reading each file. `grep` defaults to `outputMode: lines`, returning matching line numbers grouped by file (path + 1-based line number) with no line text — compact and flat, so broad searches stay small; jump straight to a location with `read`. Use `outputMode: count_matches` for exact per-file occurrence counts to spot hotspots, and when results are paged, pass `nextOffset` back as `offset` instead of re-searching. Before concluding that content does not exist, check `skipped`; search an explicit `path` when generated directories or files over 10 MB are relevant.\n" +
