...
- `components/`:
  ...
  - 弹层与小组件: `ToolsPopover.vue` / `McpStatusPopover.vue` / `SkillsPopover.vue` (欢迎页触发), `GitDiffModal.vue`, `FileInfoModal.vue`, `FileMentionMenu.vue`, `TokenPieChart.vue`, `AllyAvatar.vue`, `AllyWordmark.vue`, `SakuraBreeze.vue`, `DigitalWave.vue` (数码波浪全屏特效层)。
