...
**项目教训（lessons）是「错误驱动」的知识。** 用户敲 `/lesson` 命令，让模型回顾当前对话、把可复用的坑（比如「这个项目的测试必须用某个命令跑」）追加进项目里的 lessons 文件；下次对话这段内容作为提示词的一部分注入。注入是**有预算**的（只取最新的若干条、总字节数封顶，超出部分从最老的开始丢），所以写入侧就要求「一条一行、整行 100 字符以内、只记以后还会再踩的长期坑」，并在追加前顺手合并/删掉过时条目。这是比记忆更轻量的「项目级经验积累」——记忆偏个人长期知识，教训偏具体项目的纪律。

**用户档案（profile）是唯一「永远在场」的个人信息。** `~/.ally_agent/USER.md` 存跨项目恒定的事实与偏好（称呼、语言、时区、单位、沟通习惯、常用环境），全文注入每轮请求，所以有 8 KiB 上限、且**截尾保留开头**（人写的档案，最重要的在最前面）；写入侧要求同样严：一行一条、单行 100 字符以内、不写日期与历史、过时就合并或删掉。它和记忆的分工很清楚：记忆是「按需检索的索引 + 按需读取的正文」，档案是「每次都在场的一小段」；项目相关的任何内容都不许进档案。
...
- **长任务有出口**。命令超时不再被杀掉，也不再有关键词拦截：超时的命令被**收编**为后台服务继续运行（原进程、端口、滚动缓冲无缝接管，服务注册表、任务中心、service 工具三处可见），工具结果携带服务 id 并指引模型改用 service 工具交互。服务配额满时才回退为杀树报错。

**后台服务**是命令的常驻版：启动 dev server、worker 等长驻进程，输出进一个 512KB 的滚动缓冲（最多同时 32 个进程，再起一个会拿到 E_SERVICE_LIMIT；已完成的服务记录只保留最近 10 条），界面可随时查看尾部或全量。停止语义在三个平台统一：先优雅终止 → 宽限期等待（默认 3 秒，上限 30 秒）→ 强杀整个进程树（不是只杀直接子进程——shell 包着的真进程藏在树里），升级过程和失败原因都写进结果。应用退出时全部停掉，不做跨会话复活。
...
**计划任务**则是定时闹钟：用户登记「每天早上九点检查依赖更新」这类任务（支持 cron 表达式、固定间隔、一次性三种触发方式），到点后 Ally 用**全新空白上下文 + 固定工作区**独立执行。铁律：全局串行（一次只跑一个，绝不重叠）、每轮有步数与时长上限（默认一百步一小时，最多一千步二十四小时）、跑完只保留一小段摘要与错误信息（旧记录清场）。任务本身落盘在配置目录、重启后继续：间隔/cron 从启动时刻重新排期（不补跑错过的点），到点时 Ally 没开着的一次性任务标记为「已错过」而不是迟到执行，工作区已消失的任务标为「无效」且不再排期。它的执行同样不发流式广播，与聊天的关系是彻底隔离的。
...
| `~/.ally_agent/USER.md` | 用户档案 | 跨项目的个人信息与偏好，全文注入每轮请求；8 KiB 上限、超出截尾 |
| 项目内 `LESSONS.md` | 项目教训 | 根目录的普通文件，用户可见可改、可提交进仓库 |
...
- **任务栏零打扰**（Windows）：不设进度条、不做完成闪烁——运行中的跑马灯与结束时的闪烁都被用户感知为「图标一直在闪」，现已全部移除，完成提醒只保留最小化时的系统通知。
