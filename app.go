...
func buildSystemPromptParts(allSkills []SkillDefinition, workspaceRoot, customPrompt, gitBashPath string) []systemPromptPart {
	...
	// Inject project CODEGRAPH.md (code graph) for architectural awareness
	if workspaceRoot != "" {
		if cg := loadCodeGraph(workspaceRoot); cg != "" {
			var cgBuilder strings.Builder
			cgBuilder.WriteString("\n\n# Project Code Graph\n\n<project-codegraph>\n")
			cgBuilder.WriteString(cg)
			cgBuilder.WriteString("\n</project-codegraph>\nEnd code graph.\n")
			parts = append(parts, systemPromptPart{label: "项目代码图谱 Code Graph", content: cgBuilder.String()})
		}
	}
...
// the /codegraph command. It is injected into the system prompt as a separate
// labelled part so the model can see the project architecture on every turn.
