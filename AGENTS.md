...
4. **提示词前缀单调递增，严禁瞬态注入打断缓存**:
   - 计划任务与进度完全由内置 `plan` 工具的调用与工具结果（Tool Message）自然持久化在历史中，不再向请求前缀动态注入任何瞬态计划快照；保证消息序列 100% 严格单调追加，确保最大化的 KV 前缀缓存复用。
