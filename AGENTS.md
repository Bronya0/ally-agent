...
- `review` — two-axis review (Standards vs Spec) of a branch, PR, commit range, or uncommitted workspace changes. Runs the two axes as parallel `subagent` calls to avoid context pollution, then aggregates findings without merging or reranking them. Spec source is resolved from issue references, user-provided paths, or `docs/`/`specs/`/`.scratch/` files; standards sources come from `AGENTS.md`, `CODEGRAPH.md`, `CODING_STANDARDS.md`, `CONTRIBUTING.md`, `.editorconfig`, etc. A missing spec skips the Spec axis and reports "no spec available".
