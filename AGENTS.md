...
| `ssh_credential` | Store/clear/list SSH credentials in memory only (12h TTL): passwords typed into chat or private-key file paths (`keyPath`, passed to ssh via `-i`, coexists with a password) so `remote_*` can authenticate via `SSH_ASKPASS` (BatchMode dropped only when a password is stored). Never persisted: passwords are redacted from tool-call events, sub-agent events, and saved history through `redactSSHCredentials` before emission/write (`orch_ssh_credential.go`) |
...
- Every per-Tab workspace lookup (composer label, run overlay, footer stats, git badge, explorer, attachment path hints) resolves through `tabOwnsWorkspace(tab)` / `runWorkspaceForTab(tab)` / `activeRunWorkspace`; never compare a Tab's path against `config.workspace`. Inlined `isKbTab(x) || isTempTab(x)` copies are how the KB/temp footer stats went permanently dark: `refreshFooterStats` replied for `tab.path` but `isCurrentFooterTarget()` discarded the result against `config.workspace`, so `footerStatsLoading` never cleared and the context chip stayed on its loading placeholder forever.
- The git badge and diff modal follow the caller's directory: `GetGitStatus(workspace)` / `GetGitDiff(workspace, repoPath)` take an explicit workspace (empty = active config workspace, i.e. the last chat workspace), and a KB/temp Tab passes its own path.
