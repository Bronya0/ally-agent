...
- `remote_edit` uses the same flat single-file contract as local edit — top-level `target` (SSH target plus workspace root), `path`, `version`, and `changes`, exactly one file per call with multi-file changes expressed as parallel `remote_edit` calls — and returns the same `MultiEditResult` shape with exactly one file. Remote reads share the local text-encoding boundary (`read.DecodeTextBytes`), so UTF-16 LE/BE files are read and edited remotely exactly like locally, with UTF-8 write-back. Truncated `remote_edit` arguments recover their complete prefix through the same `salvageEditRequest()` path as `edit`, and `remote_edit` shares `edit`'s model-facing result compaction.
